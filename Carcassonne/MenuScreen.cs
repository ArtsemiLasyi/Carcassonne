...
namespace Carcassonne
{
    public class MenuScreen : Screen
    {
        ...
        {
            GraphObject mainMenuBackground = new GraphObject("BACKGROUND", Content.Load<Texture2D>(GameGlobals.TEXTUREMAINBACKGROUND), Vector2.Zero, Color.White, false);
            GraphObject mainMenuGameName = new GraphObject("LOGO", Content.Load<Texture2D>(GameGlobals.TEXTURELOGO), Vector2.Zero, Color.White, false);
            mainMenuGameName.Position = new Vector2(WIDTH / 2 - (mainMenuGameName.Texture.Width / 2), 0);
            GraphObject mainMenuTxtStartGame = new GraphObject("START", Content.Load<Texture2D>(GameGlobals.TEXTURENEWGAME), Vector2.Zero, Color.Black, true);
            mainMenuTxtStartGame.Position = new Vector2(WIDTH / 2 - (mainMenuTxtStartGame.Texture.Width / 2), 2 * HEIGHT / 7);
            GraphObject mainMenuTxtHelp = new GraphObject("HELP", Content.Load<Texture2D>(GameGlobals.TEXTUREHELP), Vector2.Zero, Color.Black, true);
            mainMenuTxtHelp.Position = new Vector2(WIDTH / 2 - (mainMenuTxtHelp.Texture.Width / 2), 4 * HEIGHT / 7);
            GraphObject mainMenuTxtExit = new GraphObject("EXIT", Content.Load<Texture2D>(GameGlobals.TEXTUREEXIT), Vector2.Zero, Color.Black, true);
